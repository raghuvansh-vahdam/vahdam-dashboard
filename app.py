...
# ── Pack-bundle groups for the combined-view card ───────────────────────────
# Per-geo registry of "virtual products" that aggregate several pack-size
# variants of the same physical product. Used by the New Business view to
# render a "Combined (Pack 1+2+3 …)" summary above the per-ASIN table.
#
# `weight` = pack size (number of physical bottles / jars per order unit).
# Net Units = Σ (order_units_for_asin × weight). Revenue / Sessions /
# Spend / etc. are simple sums; ASP and CR% are recomputed against the
# Net Units denominator so they reflect per-bottle economics.
#
# Add a new bundle by appending to the relevant geo's list. Order of the
# ASIN list dictates the order shown in the breakdown.
_NB_BUNDLES = {
    "USA": [
        {
            "key":   "turmeric_curcumin_2000mg",
            "label": "Turmeric Curcumin 2000mg — All Packs",
            "asins": [
                ("B0FLDWW2HL", 1, "Pack of 1"),
                ("B0FQV2Q4TF", 2, "Pack of 2"),
                ("B0FQV283VQ", 3, "Pack of 3"),
            ],
        },
    ],
}
...
def _nb_compute_bundle(summary_df, bundle):
    """Return a dict of combined metrics for one bundle. `summary_df` is
    the per-ASIN DataFrame returned by `get_nb_asin_summary`. Returns
    None when fewer than 1 of the bundle's ASINs are present."""
    members = {asin: (weight, label) for asin, weight, label in bundle["asins"]}
    rows = summary_df[summary_df["ASIN"].isin(members.keys())].copy()
    if rows.empty:
        return None

    # Decorate rows with per-ASIN pack-size + label for the breakdown.
    rows["PACK_SIZE"] = rows["ASIN"].map(lambda a: members[a][0])
    rows["PACK_LABEL"] = rows["ASIN"].map(lambda a: members[a][1])

    # Net Units = sum of order_units × pack_size (= total bottles/jars).
    net_units    = float((rows["UNITS"] * rows["PACK_SIZE"]).sum())
    order_units  = float(rows["UNITS"].sum())
    revenue      = float(rows["REVENUE"].sum())
    sessions     = float(rows["SESSIONS"].sum())
    total_spend  = float(rows["TOTAL_SPEND"].sum())
    ad_spend     = float(rows["AD_SPEND"].sum())
    paid_rev     = float(rows["PAID_REV"].sum())
    impressions  = float(rows["IMPRESSIONS"].sum())
    clicks       = float(rows["CLICKS"].sum())
    cm2_abs      = float(rows["CM2_ABS"].sum())
    # Inventory is also weighted: each pack-of-N box contains N bottles,
    # so the net "bottles on hand" follows the same weighting as units.
    inventory_net = float((rows.get("TOTAL_INV", pd.Series([0]*len(rows)))
                            * rows["PACK_SIZE"]).sum()) \
                    if "TOTAL_INV" in rows.columns else 0.0

    # Derived ratios. ASP + CR% use Net Units (per-bottle economics);
    # ACoS%, CM2%, CTR%, CPC use revenue/clicks as usual.
    asp        = (revenue / net_units) if net_units else 0.0
    cr_pct     = (net_units / sessions * 100) if sessions else 0.0
    acos_pct   = (total_spend / revenue * 100) if revenue else 0.0
    cm2_pct    = (cm2_abs / revenue * 100) if revenue else 0.0
    ctr_pct    = (clicks / impressions * 100) if impressions else 0.0
    cpc        = (ad_spend / clicks) if clicks else 0.0

    return {
        "label":         bundle["label"],
        "rows":          rows,                   # per-ASIN breakdown
        "net_units":     net_units,
...
def render_new_business():
    ...
    # ── Pack-bundle combined view (e.g. USA Turmeric Curcumin Pack 1+2+3) ──
    # Renders ABOVE the per-ASIN summary table so users can see weighted
    # Net-Units economics for multi-pack variants without scrolling. Only
    # appears when the current geo has bundles defined and at least one
    # member ASIN has data in the current period.
    _bundles_for_geo = _NB_BUNDLES.get(geo, [])
    for _bundle in _bundles_for_geo:
        _cb = _nb_compute_bundle(summary, _bundle)
        if not _cb:
            continue
        _cs = ("₹" if use_inr else "")
        st.markdown(
            f'<div class="section-hdr" style="margin-top:18px;">'
            f'🧮 Combined: {_cb["label"]} '
            f'<span style="font-size:12px;color:#7a6a50;font-weight:500;">'
            f'— Net Units = Σ (Units × pack size); ASP, CR%, ACoS%, CM2% '
            f'recomputed against the combined totals'
            f'</span></div>',
            unsafe_allow_html=True)
        # ── KPI strip cards (top row) ────────────────────────────────────
        _r1 = st.columns(5, gap="small")
        _r1[0].markdown(strip_card(
            "Net Units",
            f"{int(_cb['net_units']):,}",
            f"{int(_cb['order_units']):,} orders × pack size"),
            unsafe_allow_html=True)
        _r1[1].markdown(strip_card(
            "Revenue",
            f"{_cs}{_cb['revenue']:,.0f}",
            f"vs ad spend {_cs}{_cb['total_spend']:,.0f}"),
            unsafe_allow_html=True)
        _r1[2].markdown(strip_card(
            "ASP",
            f"{_cs}{_cb['asp']:,.2f}",
            "per bottle (Revenue ÷ Net Units)"),
            unsafe_allow_html=True)
        _r1[3].markdown(strip_card(
            "Sessions",
            f"{int(_cb['sessions']):,}",
            "Σ of de-overlapped sessions across pack ASINs"),
            unsafe_allow_html=True)
        _r1[4].markdown(strip_card(
            "CR%",
            f"{_cb['cr_pct']:.2f}%",
            "Net Units ÷ Sessions × 100"),
            unsafe_allow_html=True)
        # ── KPI strip cards (second row) ─────────────────────────────────
        _r2 = st.columns(5, gap="small")
        _r2[0].markdown(strip_card(
            "ACoS%",
            f"{_cb['acos_pct']:.1f}%",
            f"(PM + GADS) ÷ Revenue"),
            unsafe_allow_html=True)
        _r2[1].markdown(strip_card(
            "CM2 Abs",
            f"{_cs}{_cb['cm2_abs']:,.0f}",
            f"CM2% = {_cb['cm2_pct']:.1f}%"),
            unsafe_allow_html=True)
        _r2[2].markdown(strip_card(
            "CTR%",
            f"{_cb['ctr_pct']:.2f}%",
            f"{int(_cb['clicks']):,} clicks · "
            f"{int(_cb['impressions']):,} impr"),
            unsafe_allow_html=True)
        _r2[3].markdown(strip_card(
            "CPC",
            f"{_cs}{_cb['cpc']:,.2f}",
            "Ad Spend ÷ Clicks"),
            unsafe_allow_html=True)
        _r2[4].markdown(strip_card(
            "Net Inventory",
            f"{int(_cb['inventory_net']):,}",
            "Σ (FBA+ADW × pack size) · bottles"),
            unsafe_allow_html=True)
        # ── Per-ASIN breakdown so users can see each pack's contribution ──
        _rows_disp = _cb["rows"].copy()
        _rows_disp["Pack"]            = _rows_disp["PACK_LABEL"]
        _rows_disp["Units (orders)"]  = _rows_disp["UNITS"].astype(int)
        _rows_disp["Net Units"]       = (_rows_disp["UNITS"]
                                          * _rows_disp["PACK_SIZE"]).astype(int)
        _rows_disp["Revenue"]         = _rows_disp["REVENUE"].astype(float)
        _rows_disp["Sessions"]        = _rows_disp["SESSIONS"].astype(int)
        _rows_disp["Ad Spend"]        = _rows_disp["AD_SPEND"].astype(float)
        _rows_disp["CM2 Abs"]         = _rows_disp["CM2_ABS"].astype(float)
        if "TOTAL_INV" in _rows_disp.columns:
            _rows_disp["Inv"]         = _rows_disp["TOTAL_INV"].astype(int)
            _rows_disp["Net Inv"]     = (_rows_disp["TOTAL_INV"]
                                          * _rows_disp["PACK_SIZE"]).astype(int)
        _bd_cols = ["ASIN", "PRODUCT_NAME", "Pack",
                    "Units (orders)", "Net Units", "Revenue",
                    "Sessions", "Ad Spend", "CM2 Abs"]
        if "Inv" in _rows_disp.columns:
            _bd_cols += ["Inv", "Net Inv"]
        _rows_disp = _rows_disp[_bd_cols].rename(columns={
            "PRODUCT_NAME": "Product"})
        st.dataframe(
            _rows_disp,
            use_container_width=True, hide_index=True,
            column_config={
                ...
                "Units (orders)": st.column_config.NumberColumn(format="%,d"),
                "Net Units":      st.column_config.NumberColumn(
                    format="%,d",
                    help="Order units × pack size = bottles shipped."),
                "Revenue":        st.column_config.NumberColumn(format=f"{_cs}%,.0f"),
                "Sessions":       st.column_config.NumberColumn(format="%,d"),
                "Ad Spend":       st.column_config.NumberColumn(format=f"{_cs}%,.0f"),
                "CM2 Abs":        st.column_config.NumberColumn(format=f"{_cs}%,.0f"),
                "Inv":            st.column_config.NumberColumn(format="%,d"),
                "Net Inv":        st.column_config.NumberColumn(format="%,d"),
            },
            key=f"nb_bundle_{geo}_{_bundle['key']}",
        )
        st.markdown("")  # spacer before the section header below
